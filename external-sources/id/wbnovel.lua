...
if bp_ok and bp then
    print("[wbnovel] ✓ Bypass loaded")
else
    print("[wbnovel] ⚠ Direct HTTP mode")
end
...
language = "id"
icon     = "https://wbnovel.com/wp-content/uploads/2019/01/cropped-w-32x32.png"
...
for _, a in ipairs(html_select(r, selector)) do
    ...
    local cover = html_attr(get_element_html(a), "img", "src") or ""
...
function getBookCoverImageUrl(bookUrl)
  local body = fetchPage(bookUrl)
  if not body then return nil end
  local cover = html_attr(body, ".profile-manga .summary_image > a img", "src")
