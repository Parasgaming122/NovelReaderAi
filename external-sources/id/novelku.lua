...
if bp_ok and bp then
    print("[novelku] ✓ Bypass loaded")
else
    print("[novelku] ⚠ Direct HTTP mode")
end
...
language = "id"
icon     = "https://novelku.id/wp-content/uploads/2022/03/cropped-fvc-novelku.id_-32x32.png"
...
if not r then return { items = {}, hasNext = false } end
  ...
  local selector
  if isSearch then
    selector = ".c-tabs-item__content .tab-thumb a"
  else
    selector = "div.page-item-detail .item-thumb a"
  end
...
local isLast = html_select_first(r, ".nav-previous") == nil
...
function getBookTitle(bookUrl)
  ...
  local el = html_select_first(body, "h1.entry-title, #manga-title")
...
function getBookCoverImageUrl(bookUrl)
  local body = fetchPage(bookUrl)
  if not body then return nil end
  local cover = html_attr(body, ".summary_image img", "data-src")
...
-- Chapter list (AJAX endpoint - POST style but using GET)
...
for _, li in ipairs(html_select(r, "li.wp-manga-chapter")) do
  ...
  local el = html_select_first(cleaned, ".read-container .text-left, .chapter-content, #chapter-content")
  if not el then return "" end
  return applyStandardContentTransforms(html_text(el.html))
end

function getChapterTitle(html, url)
  local el = html_select_first(html, ".container#chapter-heding, h1.chapter-title")
  return el and string_clean(el.text) or nil
end
