...
if bp_ok and bp then
    print("[indowebnovel] ✓ Bypass loaded")
else
    print("[indowebnovel] ⚠ Direct HTTP mode")
end
...
local function applyStandardContentTransforms(text)
  ...
  text = string.gsub(text, "(?im)^\\s*(Penerjemah|Editor|Proofreader|Baca\\s+(di|di+sini|novel|lanjutan)|Sumber|Donasi|Trakteer|Dukung\\s+kami)[:\\s][^\\n\\r]{0,70}(\\r?\\n|$)", "")
  text = string.gsub(text, "(?im)^\\s*(Sponsored|Advertisement|Ads)\\s*by[^\\n\\r]{0,70}(\\r?\\n|$)", "")
...
local el = html_select_first(r, ".series-chapterlist li:first-child a")
