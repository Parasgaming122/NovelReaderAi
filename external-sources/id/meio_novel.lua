...
if bp_ok and bp then
    print("[meio_novel] ✓ Bypass loaded")
else
    print("[meio_novel] ⚠ Direct HTTP mode")
end
...
language = "id"
icon     = "https://meionovel.id/wp-content/uploads/2021/01/cropped-logoa-sa-32x32.png"
...
if not r then return { items = {}, hasNext = false } end
  ...
  if isSearch then
    ...
    selector = ".tab-content-wrap .page-item-detail .item-thumb a"
  end
...
local isLast = html_select_first(r, ".paging-navigation .nav-previous") == nil
...
function getBookCoverImageUrl(bookUrl)
  local body = fetchPage(bookUrl)
  if not body then return nil end
  local cover = html_attr(body, ".tab-summary > .summary_image img", "data-src")
...
for _, li in ipairs(html_select(r, "li.wp-manga-chapter")) do
  ...
  -- Remove h1 heading from reading content
  cleaned = -- html_remove not available, using gsub cleanup: cleaned, "h1:first-child")
  
  local el = html_select_first(cleaned, ".reading-content, .chapter-content, #chapter-content")
  if not el then return "" end
  return applyStandardContentTransforms(html_text(el.html))
end

function getChapterTitle(html, url)
  local el = html_select_first(html, ".reading-content h1, h1.chapter-title")
  return el and string_clean(el.text) or nil
end
