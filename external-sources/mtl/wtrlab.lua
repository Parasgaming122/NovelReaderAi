...
if not r then
    ...
    local items = {}
for _, card in ipairs(html_select(r, "div.series-list [data-slot='card']")) do
...
for _, block in ipairs(html_select(r, "div.items-center.text-center")) do
...
local function cleanParagraph(text)
    ...
    text = string.gsub(text, "(?i)\\A[\\s\\uFEFF]*((Глава\\s+\\d+|Chapter\\s+\\d+)[^\\n\\r]*[\\n\\r\\s]*)+", "")
...
function getChapterText(html, chapterUrl)
    ...
    local r = http_post(baseUrl .. "api/reader/get", requestBody, {
        headers = {
            ["Content-Type"] = "application/json",
            ["Referer"] = chapterUrl,
            ["Origin"] = -- regex_replace replaced by gsub: string.gsub(baseUrl, "/$", "")
        }
    })
