...
if bp_ok and bp then
    print("[rayforboe] ✓ Bypass loaded")
else
    print("[rayforboe] ⚠ Direct HTTP mode")
end
...
function getCatalogList(index)
  local url = baseUrl .. "novel/" .. tostring(index + 1) .. "/"
  local r = fetchPage(url)
if not r then return { items = {}, hasNext = false } end

  local items = {}
for _, li in ipairs(html_select(r, ".novel-list li, .book-list li")) do
...
if not r then return { items = {}, hasNext = false } end
  ...
  local items = {}
for _, li in ipairs(html_select(r, ".result-list li, .novel-list li")) do
    local titleEl = html_select_first(li.html, "h2 a, h3 a")
    if titleEl then
      table.insert(items, {
        title = string_clean(titleEl.text),
