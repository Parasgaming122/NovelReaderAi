...
if bp_ok and bp then
    print("[soxs] ✓ Bypass loaded")
else
    print("[soxs] ⚠ Direct HTTP mode")
end
...
if not r then return { items = {}, hasNext = false } end
  ...
  local items = {}
for _, li in ipairs(html_select(r, ".xiaoshuo li, .book-list li")) do
...
function getCatalogSearch(index, query)
  ...
  local url = baseUrl .. "search.php?keyword=" .. url_encode(query)
  local r = fetchPage(url)
if not r then return { items = {}, hasNext = false } end

  local items = {}
for _, li in ipairs(html_select(r, ".xiaoshuo li, .result li")) do
    local titleEl = html_select_first(li.html, "h2 a, h3 a")
    if titleEl then
      table.insert(items, {
        title = string_clean(titleEl.text),
...
function getBookTitle(bookUrl)
  ...
  local el = html_select_first(body, ".xiaoshuo h1, h1.title")
...
function getChapterList(bookUrl)
  ...
  local chapters = {}
  for _, a in ipairs(html_select(body, ".chapter-list a, #catalog a")) do
