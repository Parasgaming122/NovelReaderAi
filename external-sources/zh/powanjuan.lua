...
name     = "破万卷"
version  = "1.0.0"
baseUrl  = "https://www.powanjuan.cc/"
language = "zh"
charset  = "GB2312"
...
if bp_ok and bp then
    print("[powanjuan] ✓ Bypass loaded")
else
    print("[powanjuan] ⚠ Direct HTTP mode")
end
...
function getCatalogList(index)
  local url = baseUrl .. "sort/" .. tostring(index + 1) .. ".html"
  local r = fetchPage(url, { charset = "GB2312" })
if not r then return { items = {}, hasNext = false } end

  local items = {}
for _, li in ipairs(html_select(r, ".catalog ul.clearfix li")) do
    local titleEl = html_select_first(li.html, "a")
    if titleEl and titleEl.href ~= "" then
      table.insert(items, {
        title = string_clean(titleEl.text),
        url   = absUrl(titleEl.href),
        cover  = absUrl(html_attr(li.html, "a img", "src"))
      })
    end
  end
...
function getCatalogSearch(index, query)
  ...
  local url = baseUrl .. "modules/article/search.php?searchkey=" .. url_encode_charset(query, "GB2312") .. "&searchtype=articlename"
  local r = fetchPage(url, { charset = "GB2312" })
if not r then return { items = {}, hasNext = false } end
...
function getBookTitle(bookUrl)
  ...
  local el = html_select_first(body, ".desc h1")
  return el and string_clean(el.text):gsub("%(.*$", "") or nil
end

function getBookCoverImageUrl(bookUrl)
  local body = fetchPage(bookUrl)
  if not body then return nil end
  local cover = html_attr(body, ".cover img, .bookimg img", "src")
...
function getBookDescription(bookUrl)
  local body = fetchPage(bookUrl)
  if not body then return nil end
  local el = html_select_first(body, ".descInfo p, .intro, .desc p")
...
function getChapterList(bookUrl)
  ...
  local chapters = {}
  for _, a in ipairs(html_select(body, ".catalog ul.clearfix li a")) do
...
function getChapterListHash(bookUrl)
  local r = fetchPage(bookUrl, { charset = "GB2312" })
if not r then return nil end
local el = html_select_first(r, ".catalog ul.clearfix li:last-child a")
