...
if bp_ok and bp then
    print("[shw5] ✓ Bypass loaded")
else
    print("[shw5] ⚠ Direct HTTP mode")
end
...
for _, li in ipairs(html_select(r, ".book-list li, .list li")) do
    local titleEl = html_select_first(li.html, "h2 a, h3 a, .title a")
    if titleEl then
      table.insert(items, {
...
if not r then return { items = {}, hasNext = false } end
  ...
  local items = {}
for _, li in ipairs(html_select(r, ".book-list li, .result li")) do
...
function getBookTitle(bookUrl)
  ...
  local el = html_select_first(body, ".book h1, h1.title")
...
function getBookCoverImageUrl(bookUrl)
  local body = fetchPage(bookUrl)
  if not body then return nil end
  local cover = html_attr(body, ".cover img, .book img", "src")
...
function getBookDescription(bookUrl)
  local body = fetchPage(bookUrl)
  if not body then return nil end
  local el = html_select_first(body, ".desc p, .intro, .description")
...
function getChapterList(bookUrl)
  ...
  local chapters = {}
  for _, a in ipairs(html_select(body, ".chapter-list a, #catalog a, .catalog a")) do
...
local el = html_select_first(r, ".chapter-list a:last-child, #catalog a:last-child")
