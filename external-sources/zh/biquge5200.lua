...
if bp_ok and bp then
    print("[biquge5200] ✓ Bypass loaded")
else
    print("[biquge5200] ⚠ Direct HTTP mode")
end
...
if not r then return { items = {}, hasNext = false } end
  ...
  local items = {}
for _, li in ipairs(html_select(r, ".book-list li, #main li")) do
...
if not r then return { items = {}, hasNext = false } end
  ...
  local items = {}
for _, li in ipairs(html_select(r, ".result-list li, .book-list li")) do
    local titleEl = html_select_first(li.html, "h2 a, h3 a")
    if titleEl then
      table.insert(items, {
        title = string_clean(titleEl.text),
...
function getBookTitle(bookUrl)
  ...
  local el = html_select_first(body, "h1.title, .info h1, #info h1")
...
function getBookCoverImageUrl(bookUrl)
  local body = fetchPage(bookUrl)
  if not body then return nil end
  local cover = html_attr(body, ".cover img, .fm img, #fm img", "src")
...
function getBookDescription(bookUrl)
  local body = fetchPage(bookUrl)
  if not body then return nil end
  local el = html_select_first(body, "#intro, .intro, .desc, .intro p")
...
function getChapterList(bookUrl)
  ...
  local chapters = {}
  for _, a in ipairs(html_select(body, "#list a, dd a, .chapter-list a")) do
...
local el = html_select_first(r, "#list a:last-child, dd a:last-child")
