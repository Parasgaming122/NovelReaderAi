...
if bp_ok and bp then
    print("[syosetu] ✓ Bypass loaded")
else
    print("[syosetu] ⚠ Direct HTTP mode")
end
...
function getCatalogFiltered(index, filters)
    ...
    local r = fetchPage(url, { headers = HEADERS })
if not r then return { items = {}, hasNext = false } end

    local items = {}
for _, container in ipairs(html_select(r, ".p-ranklist-item")) do
...
local nextLink = html_select_first(r, "a.nextlink")
...
function getBookTitle(bookUrl)
    local r = fetchPage(bookUrl, { headers = HEADERS })
if not r then return nil end
local el = html_select_first(r, ".p-novel__title")
...
function getBookDescription(bookUrl)
    local r = fetchPage(bookUrl, { headers = HEADERS })
if not r then return nil end
local el = html_select_first(r, "#novel_ex")
    if el and string_trim(el.text) ~= "" then return string_trim(el.text) end
    el = html_select_first(r, ".p-novel__summary")
    return el and string_trim(el.text) or nil
end
...
local hasText = html_select_first(r, ".p-novel__text")
local hasEplist = html_select_first(r, ".p-eplist")
    
    if hasText and not hasEplist then
local titleEl = html_select_first(r, ".p-novel__title")
    ...
    -- Пагинация
    local totalPages = 1
local lastLink = html_select_first(r, ".c-pager__item--last")
    ...
    for p = 2, totalPages do
        sleep(300)
        local pr = fetchPage(bookUrl .. "?p=" .. tostring(p), { headers = HEADERS })
...
function getChapterListHash(bookUrl)
    local r = fetchPage(bookUrl, { headers = HEADERS })
if not r then return nil end
local el = html_select_first(r, ".p-novel__date-published")
    return el and string_trim(el.text) or nil
end
