...
if bp_ok and bp then
    print("[boxnovel] ✓ Bypass loaded")
else
    print("[boxnovel] ⚠ Direct HTTP mode")
end
...
-- Catalog (WordPress manga theme with alphabet ordering)
function getCatalogList(index)
  local page = index + 1
  local url = baseUrl .. "novel/?m_orderby=alphabet"
  if page > 1 then
...
if not r then return { items = {}, hasNext = false } end
  ...
  local items = {}
  -- BoxNovel uses .page-item-detail pattern for catalog items
for _, item in ipairs(html_select(r, ".page-item-detail")) do
    local linkEl = html_select_first(item.html, "a[href]")
    if linkEl then
      ...
      local cover = html_attr(item.html, "img[data-src]", "data-src")
                 or html_attr(item.html, "img", "src")
                 or ""
...
if not r then return { items = {}, hasNext = false } end
  ...
  local items = {}
  -- Search results use .c-tabs-item__content pattern
for _, item in ipairs(html_select(r, ".c-tabs-item__content")) do
    local linkEl = html_select_first(item.html, "a[href]")
    if linkEl then
      local title = linkEl.attr.title or string_clean(linkEl.text)
      local href = absUrl(linkEl.href)
...
function getBookCoverImageUrl(bookUrl)
  ...
  local cover = html_attr(body, "div.summary_image img[data-src]", "data-src")
         or html_attr(body, "div.summary_image img", "src")
...
function getBookDescription(bookUrl)
  local body = fetchPage(bookUrl)
  if not body then return nil end
  local el = html_select_first(body, ".summary__content.show-more, .summary__content .show-more, .summary__content")
  return el and string_trim(html_text(el.html)) or nil
end

-- Chapter list (AJAX endpoint /ajax/chapters/)
function getChapterList(bookUrl)
  ...
  local chapters = {}
  -- WordPress manga chapter links
  for _, a in ipairs(html_select(r.body, ".wp-manga-chapter > a[href]")) do
...
function getChapterListHash(bookUrl)
  ...
  local el = html_select_first(r.body, ".wp-manga-chapter:last-child a")
