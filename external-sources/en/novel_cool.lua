...
if bp_ok and bp then
    print("[novel_cool] ✓ Bypass loaded")
else
    print("[novel_cool] ⚠ Direct HTTP mode")
end
...
language = "en"
icon     = "https://www.novelcool.com/favicon.ico"

-- NOTE: As of 2026-06-30, this site may return HTTP 404 (server behind CF down).
-- Scraper based on archived HTML structure from Wayback Machine (2026-06-13).
...
function getCatalogList(index)
  local page = index + 1
  local url = baseUrl .. "category/latest.html"
...
if not r then return { items = {}, hasNext = false } end
  ...
  local items = {}
  local seenUrls = {} -- Track URLs to avoid duplicates
...
for _, a in ipairs(html_select(r, "a[href*='/novel/']")) do
    ...
    if href and href ~= "" then
      ...
      if not string_match(href, "_%d+%.html$") then
        ...
        -- Get title from attribute or child elements
        local title = html_attr_raw(a, "title")
        if not title or title == "" then
          local titleEl = html_select_first(get_element_html(a), "h2, h3, h4, .title, .book-name")
          title = titleEl and string_clean(titleEl.text) or ""
        end
        title = string_trim(title)
        
        -- Skip if too short or duplicate URL
...
function getCatalogSearch(index, query)
  ...
  local page = index + 1
  local url = baseUrl .. "search/?searchkey=" .. url_encode(query)
  if page > 1 then url = url .. "&page=" .. tostring(page) end
...
function getBookCoverImageUrl(bookUrl)
  local body = fetchPage(bookUrl)
  if not body then return nil end
  local cover = html_attr(body, "img.bookinfo-pic-img[src]", "src")
    or html_attr(body, ".bookinfo-pic img[src]", "src")
...
function getBookDescription(bookUrl)
  local body = fetchPage(bookUrl)
  if not body then return nil end
  local descEl = html_select_first(body, "div.bk-summary-txt, div.bk-summary")
  if not descEl then return nil end
  -- Remove headers
  local cleaned = html
  
  local el = html_select_first(cleaned, ".chapter-content, #chapter-content, .read-content, #content, .reading-content, article.content")
