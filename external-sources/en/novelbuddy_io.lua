...
if bp_ok and bp then
    print("[novelbuddy] ✓ Bypass loaded")
else
    print("[novelbuddy] ⚠ Direct HTTP mode")
end
...
local function cleanContent(text)
    ...
    -- Убираем watermarks
    text = string.gsub(text, "(?i)Find authorized novels in Webnovel.*?Please click www\\.webnovel\\.com for visiting\\.", "")
    text = string.gsub(text, "(?i)free.{0,10}novel\\.com", "")
...
local function apiGet(path)
    local r = fetchPage(API_BASE .. path)
...
function getChapterText(html, url)
    ...
    content = string.gsub(content,
        ...
        "")
    content = string.gsub(content, "(?i)free.*?novel\\.com", "")
