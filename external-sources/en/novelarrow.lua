...
if bp_ok and bp then
    print("[NovelArrow] ✓ Bypass loaded")
else
    print("[NovelArrow] ⚠ Direct HTTP mode")
end
...
local function fetchBookData(novelId)
    ...
    local r = fetchPage(apiBase .. "novels/" .. novelId)
if not r then
...
local function stripHtml(html)
    ...
    local text = -- regex_replace replaced by gsub: string.gsub(html, "<[^>]*>", "")
...
local function applyStandardContentTransforms(text)
    ...
    text = string_normalize(text)
    text = string.gsub(text, "(?i)novelarrow\\.com.*?\\n", "")
...
if not r then
    ...
    local cleaned = -- html_remove not available, using gsub cleanup: content, "h3", "script", "style", ".ads")
