...
if bp_ok and bp then
    print("[novelfrance] ✓ Bypass loaded")
else
    print("[novelfrance] ⚠ Direct HTTP mode")
end
...
function getCatalogSearch(index, query)
    ...
    local r = fetchPage(url, { headers = browserHeaders })
    if not r or not r.success then return { items = {}, hasNext = false } end
...
function getChapterList(bookUrl)
    ...
    while hasMore do
        ...
        local r = fetchPage(apiUrl, { headers = apiHeaders })
...
function getChapterText(html, chapterUrl)
    ...
    local r = fetchPage(baseUrl .. "/api/chapters/" .. apiPath, { headers = apiHeaders })
...
local function getChapterTextFromHtml(html)
    ...
    text = string.gsub(text, "(?i)\\A[\\s\\p{Z}\\uFEFF]*Chapitre\\s+\\d+[^\\n\\r]*[\\n\\r\\s]*", "")
    text = string.gsub(text, "(?i)^\\s*\\d+\\s+[^\\n\\r]{0,100}[\\n\\r]", "")
    text = string.gsub(text, "(?i)Si vous voulez avoir plus d'infos.*?discord\\.gg/[^\\s]*", "")
