...
const BASE = 'https://www.shw5.cc';

export class Shw5Plugin implements NovelSourcePlugin {
  public info: PluginSourceInfo = {
    ...
    language: 'zh',
    version: '1.0.0',
    icon: 'https://raw.githubusercontent.com/Parasgaming122/external-sources/main/icons/shw5.png',
    hasSearch: true,
    charset: 'UTF-8',
  ...
  async getBookDetails(bookUrl: string): Promise<PluginNovelDetail | null> {
    ...
    const $ = cheerio.load(html);
    const title = $('.book h1, h1.title').first().text().trim();
    if (!title) return null;

    const cover = $('.cover img, .book img').first().attr('src');
    const summary = $('.desc p, .intro, .description').first().text().trim();

    const chapters: PluginChapterItem[] = [];
    $('.chapter-list a, #catalog a, .catalog a').each((_, el) => {
