...
const BASE = 'https://www.soxs.cc';

export class SoxsPlugin implements NovelSourcePlugin {
  public info: PluginSourceInfo = {
    ...
    language: 'zh',
    version: '1.0.0',
    icon: 'https://raw.githubusercontent.com/Parasgaming122/external-sources/main/icons/soxs.png',
    hasSearch: true,
    charset: 'UTF-8',
  ...
  async getCatalogList(page = 1): Promise<{ items: PluginNovelItem[]; hasNext: boolean }> {
    ...
    $('.xiaoshuo li, .book-list li').each((_, el) => {
      const $el = $(el);
  ...
  async getCatalogSearch(query: string, page = 1): Promise<{ items: PluginNovelItem[]; hasNext: boolean }> {
    const searchUrl = `${BASE}/search.php?keyword=${encodeURIComponent(query)}&page=${page}`;
    ...
    $('.xiaoshuo li, .result li').each((_, el) => {
      const $el = $(el);
      const titleEl = $el.find('h2 a, h3 a').first();
      const title = this.cleanText(titleEl.text());
      const href = titleEl.attr('href');
  ...
  async getBookDetails(bookUrl: string): Promise<PluginNovelDetail | null> {
    ...
    const $ = cheerio.load(html);
    const title = this.cleanText($('.xiaoshuo h1, h1.title').first().text());
    ...
    $('.chapter-list a, #catalog a').each((_, el) => {
      const $a = $(el);
  ...
  async getChapterText(chapterUrl: string): Promise<{ title?: string; contentHtml: string; rawText: string }> {
    ...
    const $ = cheerio.load(html);
    const title = this.cleanText($('h1.title, h1.chapter-title, h1').first().text());
