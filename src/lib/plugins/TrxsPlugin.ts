...
const BASE = 'https://trxs.cc';

export class TrxsPlugin implements NovelSourcePlugin {
  public info: PluginSourceInfo = {
    ...
    language: 'zh',
    version: '1.0.0',
    icon: 'https://raw.githubusercontent.com/Parasgaming122/external-sources/main/icons/trxs.png',
    hasSearch: false,
    charset: 'UTF-8',
    description: '天人小说 — Chinese web novel reading platform. Browse catalog and search.',
    blocked: true,
    blockedReason: 'Site now a fan-fiction (同人小说) site in GB2312 encoding — not general novels',
    cfBlockLevel: 'full',
    cfStatus: 'Site repurposed as fan-fiction only. Not suitable for general novel search.',
  ...
  async getCatalogSearch(query: string, page = 1): Promise<{ items: PluginNovelItem[]; hasNext: boolean }> {
    ...
    $('.book-list li, .result li').each((_, el) => {
      const $el = $(el);
      const titleEl = $el.find('h2 a, h3 a').first();
      const title = this.cleanText(titleEl.text());
      const href = titleEl.attr('href');
